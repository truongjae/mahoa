...
def nghich_dao(a):
	...
	if math_det_a != 0:
		if b > 2:
			...
			for i in range(b):
				for j in range(b):
					#chuyenvi2.append(math_det(chuyenvi[i][j])*((-1)**(i+j))/math_det_a)
					chuyenvi2.append(math_det(chuyenvi[i][j])*((-1)**(i+j)))
...
def decode(t,k,m):
	...
	print("det(k) =",math_det(matran_k))
	key=draw_table(math_det(matran_k))
...
if choice == "1":
	...
	a1= int(input("nhập vào a[1][1]: "))
	a2= int(input("nhập vào a[1][2]: "))
	a3= int(input("nhập vào a[2][1]: "))
	a4= int(input("nhập vào a[2][2]: "))
	cap = int(input("nhập vào cấp ma trận: "))
	encode(code,[a1,a2,a3,a4],cap)
elif choice == "2":
